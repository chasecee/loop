"""Route registration for LOOP web server - WebSocket-free polling architecture."""
...
def register_routers(app, display_player=None, wifi_manager=None, updater=None, config=None):
    """Register all routers with the FastAPI app - no WebSocket complexity."""
    ...
    # Create and register all routers (WebSocket-free)
    ...
    # Add simple polling endpoint for frontend
    from .polling import create_polling_router
    app.include_router(create_polling_router(display_player, wifi_manager, updater)) 